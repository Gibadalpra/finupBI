...
const Header = ({ user, onMenuToggle, sidebarCollapsed }) => {
  ...
  const handleProfileSettings = () => {
    ...
    setShowUserDropdown(false); // Fecha o dropdown após navegar
  };

  const notifications = [
    { id: 1, message: 'Conciliação bancária concluída', time: 'há 5 min', type: 'success' },
    { id: 2, message: 'Prazo para declaração fiscal se aproximando', time: 'há 1 hora', type: 'warning' },
    { id: 3, message: 'Nova solicitação de acesso ao portal do cliente', time: 'há 2 horas', type: 'info' },
  ];
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div className="hidden sm:block">
              <h1 className="text-xl font-heading font-bold text-primary">Finup_BI</h1>
              <p className="text-xs text-text-secondary font-medium">Business Intelligence</p>
            </div>
          </div>
        </div>
            ...
            {showNotifications && (
              <div className="absolute right-0 top-full mt-2 w-80 bg-surface rounded-xl shadow-floating border border-border z-dropdown">
                <div className="p-4 border-b border-border">
                  <h3 className="font-heading font-semibold text-text-primary">Notificações</h3>
                </div>
              ...
              <div className="absolute right-0 top-full mt-2 w-56 bg-surface rounded-xl shadow-floating border border-border z-dropdown">
                ...
                <div className="py-2">
                  <button 
                    ...
                    className="w-full px-4 py-2 text-left text-sm text-text-primary hover:bg-background nav-transition flex items-center space-x-3"
                  >
