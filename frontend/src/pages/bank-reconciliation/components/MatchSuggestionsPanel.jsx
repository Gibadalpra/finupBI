...
const MatchSuggestionsPanel = ({ 
  ...
  const formatAmount = (amount) => {
    ...
    return amount >= 0 ? `+R$${absAmount?.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}` 
                       : `-R$${absAmount?.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`;
  };
      ...
      <div className="overflow-y-auto max-h-96">
        {sortedSuggestions?.length === 0 ? (
          <div className="p-8 text-center">
            ...
            <p className="text-text-secondary">Nenhuma sugestão de correspondência disponível</p>
            <p className="text-sm text-text-secondary mt-2">
              Tente ajustar suas regras de correspondência ou importar mais transações
            </p>
          </div>
        ) : (
              ...
              >
                ...
                <div className="flex items-center justify-between mb-3">
                  <span className={`text-sm font-medium px-2 py-1 rounded-full border ${getConfidenceBadge(suggestion?.confidence)}`}>
                    {Math.round(suggestion?.confidence * 100)}% Correspondência
                  </span>
                ...
                <div className="bg-primary-50 rounded-lg p-3 mb-2">
                  <div className="flex items-center space-x-2 mb-2">
                    ...
                    <span className="text-xs font-medium text-primary">Transação Bancária</span>
                    <span className="text-xs text-text-secondary">
                      {new Date(suggestion.bankTransaction.date)?.toLocaleDateString('pt-BR')}
                    </span>
                  </div>
                ...
                <div className="bg-secondary-50 rounded-lg p-3 mb-3">
                  <div className="flex items-center space-x-2 mb-2">
                    ...
                    <span className="text-xs font-medium text-secondary">Transação Registrada</span>
                    <span className="text-xs text-text-secondary">
                      {new Date(suggestion.recordedTransaction.date)?.toLocaleDateString('pt-BR')}
                    </span>
                  </div>
                  <p className="text-sm text-text-primary mb-1">
                ...
                <div className="flex space-x-2">
                  ...
                  >
                    ...
                    <span>Aceitar Correspondência</span>
                  </button>
