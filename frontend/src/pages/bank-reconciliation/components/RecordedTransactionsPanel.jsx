...
const RecordedTransactionsPanel = ({ 
  ...
  const getAccountColor = (account) => {
    const colors = {
      'Contas a Receber': 'bg-blue-100 text-blue-700',
      'Despesas de Escritório': 'bg-orange-100 text-orange-700',
      'Despesas de Folha de Pagamento': 'bg-purple-100 text-purple-700',
      'Receita de Serviços': 'bg-green-100 text-green-700',
      'Despesas de Utilidades': 'bg-red-100 text-red-700'
    };
      ...
      <div className="p-4 border-b border-border">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-sm text-text-secondary bg-accent-100 px-2 py-1 rounded-full">
            {filteredAndSortedTransactions?.length} não conciliadas
          </span>
        </div>
      ...
      <div className="overflow-y-auto max-h-96">
        {filteredAndSortedTransactions?.length === 0 ? (
          <div className="p-8 text-center">
            ...
            <p className="text-text-secondary">Todas as transações foram conciliadas</p>
          </div>
        ) : (
