...
const UserTable = ({ 
  ...
  const getRoleColor = (role) => {
    switch (role) {
      ...
      case 'Client Admin': return 'bg-warning-100 text-warning-700';
      case 'Client User': return 'bg-orange-100 text-orange-700';
      default: return 'bg-gray-100 text-gray-700';
    }
  };

  const getUserTypeColor = (userType) => {
    return userType === 'internal' ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700';
  };

  const getUserTypeLabel = (userType) => {
    return userType === 'internal' ? 'Interno' : 'Externo';
  };
    ...
    <div className="bg-surface rounded-lg border border-border overflow-hidden">
      ...
      <div className="px-6 py-4 border-b border-border bg-background">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-heading font-semibold text-text-primary">Membros da Equipe</h3>
              ...
              >
                ...
                <td className="px-6 py-4">
                  <div className="space-y-1">
                    ...
                    {user?.userType === 'external' && user?.clientCompany && (
                      <p className="text-xs text-text-secondary">{user?.clientCompany}</p>
                    )}
                  </div>
                </td>
                <td className="px-6 py-4">
      ...
      {users?.length === 0 && (
        <div className="px-6 py-12 text-center">
          ...
          <h3 className="text-lg font-medium text-text-primary mb-2">Nenhum usuário encontrado</h3>
          <p className="text-text-secondary">Comece adicionando seu primeiro membro da equipe.</p>
        </div>
      )}
    </div>
  );
