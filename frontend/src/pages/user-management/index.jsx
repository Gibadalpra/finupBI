...
// import UserDetailPanel from './components/UserDetailPanel'; // Removido conforme solicitado
  ...
  const [users, setUsers] = useState([
    {
      ...
      role: "Partner",
      userType: "internal", // interno (da consultoria)
      clientCompany: null, // não se aplica para usuários internos
    ...
    },
    {
      ...
      role: "Freelancer",
      userType: "internal", // freelancer ainda é considerado interno
      clientCompany: null,
    ...
    },
    {
      ...
      role: "Client Admin",
      userType: "external", // externo (da empresa-cliente)
    ...
    },
    {
      id: 6,
      name: "Carlos Silva",
      email: "carlos.silva@innovacorp.com",
      role: "Client User",
      userType: "external", // externo (da empresa-cliente)
      clientCompany: "Innova Corp",
      permissions: ["Client Portal", "View Reports"],
      status: "Active",
      ...
      loginHistory: [
        { date: new Date(Date.now() - 1200000), ip: "203.0.113.67", device: "Chrome on Windows" },
        { date: new Date(Date.now() - 172800000), ip: "203.0.113.67", device: "Chrome on Windows" }
      ],
      activityLog: [
        { action: "Downloaded report", timestamp: new Date(Date.now() - 3600000) },
        { action: "Logged in", timestamp: new Date(Date.now() - 3900000) }
      ]
    },
    {
      id: 7,
      ...
      permissions: ["Client Portal", "View Reports", "User Management"],
      status: "Active",
      ...
      loginHistory: [
        { date: new Date(Date.now() - 900000), ip: "198.51.100.89", device: "Firefox on Mac" },
        { date: new Date(Date.now() - 86400000), ip: "198.51.100.89", device: "Firefox on Mac" }
      ],
      activityLog: [
        { action: "Added new user", timestamp: new Date(Date.now() - 5400000) },
        { action: "Logged in", timestamp: new Date(Date.now() - 6300000) }
      ]
    }
  ]);
        ...
        <div className="p-6">
          ...
          <div className="mb-8">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-6">
              <div>
                <h1 className="text-3xl font-heading font-bold text-text-primary mb-2">Gerenciamento de Usuários</h1>
                <p className="text-text-secondary">Gerencie membros da equipe, funções e permissões em sua organização</p>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              ...
              <div className="bg-surface rounded-lg p-4 border border-border">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-text-secondary">Usuários Internos</p>
                    <p className="text-2xl font-bold text-accent">{users?.filter(u => u?.userType === 'internal')?.length}</p>
                  </div>
              ...
              <div className="bg-surface rounded-lg p-4 border border-border">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-text-secondary">Usuários Externos</p>
                    <p className="text-2xl font-bold text-secondary">
                      {users?.filter(u => u?.userType === 'external')?.length}
                    </p>
                  </div>
